""" device_wrangler.py
    ...
    Instantiate devices and provide inital configuration
    This file can ultimately be supplemented with an external YAML file

    Notes:
      Python 3 preserves ordering in dictionaries - should this ever become important here.

      At some point it may make sense to use a YAML file, but for now this is pretty static.

      Separation-of-concerns suggests the device assignment live with their classes,
        but having them together is simple and enforces unique names across all devices.
